...
def downgrade():
    ...
    op.execute("ALTER TYPE commissionstatus RENAME TO commissionstatus_old")
    op.execute(
        "CREATE TYPE commissionstatus AS ENUM ('REQUESTED', 'UNPAID', 'DENY', 'CONFIRM', 'PAID')"
    )
    op.execute(
        "ALTER TABLE jobs ALTER COLUMN commission_status TYPE commissionstatus USING commission_status::text::commissionstatus"
    )
